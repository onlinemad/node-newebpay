...
_newebpay.prototype.CheckCode = function () {
  return Promise.resolve(crypto.createHash('sha256').update(qs.stringify(Object.assign({ HashIV: this.iv }, Object.keys(this._trade_info).sort().reduce((r, k) => (r[k] = this._trade_info[k], r), {}), { HashKey: this.key })).replace(/%20/g, '+')).digest('hex').toUpperCase())
}
...
_newebpay.prototype.CheckValue = function (type) {
  ...
  return Promise.resolve(crypto.createHash('sha256').update(str).digest('hex').toUpperCase())
}
